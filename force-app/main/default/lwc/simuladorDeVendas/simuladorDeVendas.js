...
export default class SimuladorDeVendas extends NavigationMixin(LightningElement) {
    ...
    doNextstep(){
        ...
        if (this.currentStep === 3) {
            const simuladorTelaExtrato = this.template.querySelector('c-simulador-tela-negociacao');
            console.log(simuladorTelaExtrato + ' simulador tela extrato');
            if (simuladorTelaExtrato) {
                console.log('if do simulador tela extrato');
                simuladorTelaExtrato.executarAcaoCustomizada();
            } else {
                console.warn('Componente c-simulador-tela-negociacao ainda não está renderizado');
            }
        }
